...
function chartResponse() {
    ...
    // remove and replace SVG area if it is empty when browser loads
    if (!svgArea.empty()) {
        svgArea.remove();
    }
    ...
    d3.csv("assets/data/data.csv").then(censusData => {
        ...
        // Call the updateTooltip function
        updateTooltip(xAxisFactor, yAxisFactor, circlesGroup, stateLabels);

        // Event listeners for axes changes on clicking X axis labels
        xAxesLabels.selectAll("text").on("click", function() {
            ...
            if (xAxisValue !== xAxisFactor) {
                ...
                // switch statement for updating clicked label to active and make others inactive
                switch (xAxisFactor) {
                    case "poverty":
...
// Function to update circles and labes on scatter plot 
function updatePlot(circlesGroup, stateLabels, xScale, yScale, xAxisFactor, yAxisFactor) {

    // transition new circlesGroup by changing their posiitons to new values
    circlesGroup.transition()
        .duration(1000)
        .attr("cx", d => xScale(d[xAxisFactor]))
        .attr("cy", d => yScale(d[yAxisFactor]));

    // transition new state labels by changing their posiitons to new values   
...
// When the browser window is resized, chartResponse() is called.
d3.select(window).on("resize", chartResponse);